...
const OrderToPdf = (props) => {
  ...
  return (
    <div style={{ margin: ".5em" }}>
      ...
      <div className="client-contact-info border">
        <div style={{ flexGrow: 1, padding: ".3em", fontSize: "large" }}>
          ...
          <div style={{ display: "flex", margin: ".5em" }}>
            <div style={{ marginLeft: ".5em" }}>
              رقم الهاتف : {orderInfo ? orderInfo.telephone + " , " + orderInfo.seconde_telephone	  : null}{" "}
            </div>
            <div style={{ flexGrow: 1 }}></div>
          </div>
          <div style={{ display: "flex", margin: ".5em" }}>
          ...
          <div style={{ display: "flex", margin: ".5em" }}>
            <div style={{ marginLeft: ".5em" }}>
              المجموع الكلي للطلب : {parseInt(orderTotal).toLocaleString("ar-IQ")} دينار 
            </div>
            <div style={{ flexGrow: 1 }}></div>
          </div>
      ...
      <div style={{ display: "flex", marginTop: ".5em" }}>
        <div style={{ flexGrow: 1 }}>
          <div
            style={{ marginRight: ".5em", textAlign: "center", padding: "5px" }}
            className="border"
          >
            السعر الكلي
          </div>
          ...
          >
            {orderLinesPageOne && orderLinesPageOne.length
              ? orderLinesPageOne.map((line) => {
                  return (
                    <p>
                      {parseInt(line.one_product_total).toLocaleString("ar-IQ")}
                    </p>
                  );
                })
              : null}
          ...
          <div style={{ textAlign: "center" }} className="border">
            {orderLinesPageOne && orderLinesPageOne.length
              ? orderLinesPageOne.map((line) => {
                  return (
                    <p>
                      {parseInt(line.one_product_count).toLocaleString("ar-IQ")}{" "}
                      *{" "}
                      {parseInt(line.one_product_price).toLocaleString("ar-IQ")}
                    </p>
                  );
                })
              : null}
        ...
        <div style={{ flexGrow: 5 }}>
          <div
            style={{ marginLeft: ".5em", textAlign: "center", padding: "5px" }}
            className="border"
          >
            الوصف
          </div>
          <div
            style={{ marginLeft: ".5em", textAlign: "center" }}
            className="border"
          >
            {orderLinesPageOne && orderLinesPageOne.length
              ? orderLinesPageOne.map((line) => {
                  return (
                    <p>
                      {line.product_id} :{" "}
                      {line.title ? truncate(line.title, 50) : "من غير عنوان"}{" "}
                    </p>
                  );
                })
              : null}
      ...
      {orderLinesPageTwo && orderLinesPageTwo.length ? <p>صفحة رقم 1</p> : null}
   ...
   <div style={{ display: "flex", marginTop: "1.5em" }}>
      ...
      {orderLinesPageTwo && orderLinesPageTwo.length ? <p>صفحة رقم 2</p> : null}
