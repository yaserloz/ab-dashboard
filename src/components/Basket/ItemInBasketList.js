...
import {
  updateOneProductPriceInCurrentOrder,
  updateProductPriceForCurrentOrderInDatabase,
  deleteProductLineFromSellingOrder
} from '../../store/sellingOrder';
...
export default function ItemInBasketList() {
  ...
  const deleteProductFromBasketHandler = (product) => (event) => {
    ...
    answer = window.confirm(
      'هل انت متاكد بانك تريد حذف المنتج من الوصل الحالي ؟'
    );
    if (!answer) {
      return;
    }
    dispatch(deleteProductLineFromSellingOrder(product));
  };

  const onPricChangeHandler = (index) => (event) => {
    const newPrice = event.target.value;
    dispatch(updateOneProductPriceInCurrentOrder(index, newPrice));
  };
  ...
  const updateBacketItem = (product) => (event) => {
    dispatch(updateProductPriceForCurrentOrderInDatabase(product));
  };

  return (
    <>
      {currentSellingOrder &&
      currentSellingOrder.orderLines &&
      currentSellingOrder.orderLines.length ? (
        <>
              ...
              <TableBody>
                {currentSellingOrder.orderLines.map((product, index) => (
                  <TableRow key={product.id}>
                    ...
                    <TableCell align="right">
                      {priceRow(product.count, product.unit_price)}
                    </TableCell>
                    <TableCell align="right">
                      <DeleteIcon
                    ...
                    <TableCell align="right">
                      {product.toSave ? (
                        <SaveIcon onClick={updateBacketItem(product)} />
                      ) : null}
                    </TableCell>
                  </TableRow>
                ))}
